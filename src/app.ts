...
const panel: HTMLDivElement = document.querySelector('.panel') as HTMLDivElement
...
function createHtmlElementWithClass(html_tag: string, class_name: string, id?: string): HTMLElement {
...
class Lavaka {
    ...
    constructor(id: number, owner: Player) {
        this.id = id
        this.owner = owner
        this.container = createHtmlElementWithClass("div", "lavaka") as HTMLDivElement
        this.vato_container = createHtmlElementWithClass("p", "num") as HTMLDivElement
    ...
    setIsanVato(n: number) { this.isan_vato = n; this.testDraw(2000) }
    ...
    drawArrow() {
        this.removeArrow()
        this.arrow_left = createHtmlElementWithClass("div", "arrow arrow_left") as HTMLDivElement
        this.arrow_right = createHtmlElementWithClass("div", "arrow arrow_right") as HTMLDivElement
...
class Player {
    ...
    constructor(name: string, isNorth: boolean) {
        ...
        this.table_div = createHtmlElementWithClass("div", "table") as HTMLDivElement
        panel.appendChild(this.table_div)

        this.lavaka_order.forEach(id => {
            this.lavaka_collection[id] = new Lavaka(id, this)
            this.table_div.appendChild(this.lavaka_collection[id]?.getContainer() as HTMLDivElement)
        })
    ...
    getLavaka<Type>(id?: number): Type {
        return ((id !== undefined)? this.lavaka_collection[id] : this.lavaka_collection) as Type
    }
}
...
class Game {
    ...
    private move(start: number, isRight: boolean) {
        //initialisation
        this.vato_noraisina = this.player.getLavaka<Lavaka>(start).getIsanVato()
            ...
            if(this.lanyV1() && this.nisyVatoTaoAmFarany()) {
                if(this.afakaMaka()) {
                    if(this.adveTsisyVatoEoAmbony()) {
                        this.vato_noraisina = this.player.getLavaka<Lavaka>(this.current_index).getIsanVato() + this.makaAoAm(Math.abs(this.current_index - 7))
                    } else {
                        this.vato_noraisina = this.player.getLavaka<Lavaka>(this.current_index).getIsanVato() + this.makaAoAm(this.current_index)
                    }
                    this.player.getLavaka<Lavaka>(this.current_index).setIsanVato(0)
                    start = this.current_index
                    this.steps_nbr = 0
                } else {
                    this.vato_noraisina = this.player.getLavaka<Lavaka>(this.current_index).getIsanVato()
                    this.player.getLavaka<Lavaka>(this.current_index).setIsanVato(0)
                    start = this.current_index
                    this.steps_nbr = 0
                }
    ...
    private nisyVatoTaoAmFarany() {
        return (this.player.getLavaka<Lavaka>(this.current_index).getIsanVato() != 1)
    }
    private isMandry() {
    ...
    private makaAoAm(index: number) {
        let azo: number = this.adve.getLavaka<Lavaka>(Math.abs(index)).getIsanVato()
        this.adve.getLavaka<Lavaka>(Math.abs(index)).setIsanVato(0)
        return azo
    }
    private endGame() {
...
const btn_play: HTMLElement = document.querySelector("#btn_play") as HTMLElement
const player1_field: HTMLInputElement = document.querySelector("#player1_field") as HTMLInputElement
const player2_field: HTMLInputElement = document.querySelector("#player2_field") as HTMLInputElement
const player1_name: HTMLHeadingElement = document.querySelector("#player1_name") as HTMLHeadingElement
const player2_name: HTMLHeadingElement = document.querySelector("#player2_name") as HTMLHeadingElement
const player1_score: HTMLHeadingElement = document.querySelector("#player1_score") as HTMLHeadingElement
const player2_score: HTMLHeadingElement = document.querySelector("#player2_score") as HTMLHeadingElement
